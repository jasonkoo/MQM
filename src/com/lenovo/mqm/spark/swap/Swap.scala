package com.lenovo.mqm.spark.swap
...
/*
 * Author: gulei2
 * description: swap table generation
 * 1. deduplication
 * 2. aggregateByKey
 * 3. sortAndChain
 * 4. format and output
...
object Swap {
   def main(args: Array[String]) {
     ...
     val sparkConf = new SparkConf().setAppName("Swap Table Generation")
     ...
     // 3. sortAndChain
     // (imsi, (fromimei, toimei, eventdate))
     ...
     val move = sortAndChained.map {
       case (k, v) => Array(k, "imsi", v._1, v._2, "swap", v._3).mkString("\001")
     }
